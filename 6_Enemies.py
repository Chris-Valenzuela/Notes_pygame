...
class enemy(object):
    ...
    def draw(self, win):
        ...
        if self.vel > 0:
            # flooring by divisor 3 so that images arent going to fast 
            # also there are 11 enemy images so flooring it by 3 keeps it so that we use all 11 images 
    ...
    def move(self):
        #going right
        if self.vel > 0:
            #if we are not pass the end coordinate then u can keep moving
            if self.x + self.vel < self.path[1]:
                self.x += self.vel
            # if its passed the coordiante then u flipped the velocity 
            else:
                self.vel = self.vel * - 1
                self.walkCount = 0 
        #going left
